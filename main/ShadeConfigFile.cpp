...
static const char *s_TAG = "ShadeConfigFile";
...
bool ShadeConfigFile::begin(bool readOnly)
{
    return this->begin("/shades.cfg", readOnly);
}
bool ShadeConfigFile::begin(const char *filename, bool readOnly)
{
    return ConfigFile::begin(filename, readOnly);
}
void ShadeConfigFile::end()
{
    ConfigFile::end();
}
...
bool ShadeConfigFile::validate()
{
    ...
    if (this->header.version < 1) {
        ESP_LOGI(s_TAG, "Invalid Header Version: %d", this->header.version);
        return false;
    }
    if (this->header.shadeRecordSize < 100) {
        ESP_LOGI(s_TAG, "Invalid Shade Record Size: %d", this->header.shadeRecordSize);
        return false;
    }
    /*
    if(this->header.shadeRecords != SOMFY_MAX_SHADES) {
      ESP_LOGI(s_TAG, "Invalid Shade Record Count: %d", this->header.shadeRecords);
      return false;
    }
    */
    if (this->header.version > 10) {
        if (this->header.groupRecordSize < 100) {
            ESP_LOGI(s_TAG, "Invalid Group Record Size: %d", this->header.groupRecordSize);
            return false;
        }
        /*
        if(this->header.groupRecords != SOMFY_MAX_GROUPS) {
          ESP_LOGI(s_TAG, "Invalid Group Record Count: %d", this->header.groupRecords);
          return false;
    ...
    if (this->file.position() != this->header.length) {
        ESP_LOGI(s_TAG, "File not positioned at %u end of header: %d", this->header.length, this->file.position());
        return false;
    }
    ...
    if (this->file.size() != fsize) {
        ESP_LOGI(s_TAG, "File size is not correct should be %lu and got %u", fsize, this->file.size());
    }
    ...
    if (this->header.version >= 19) {
        while (recs < this->header.roomRecords) {
            ...
            if (!this->seekChar(CFG_REC_END)) {
                ESP_LOGI(s_TAG, "Failed to find the room record end %d", recs);
                return false;
            }
            if (this->file.position() - pos != this->header.roomRecordSize) {
                ESP_LOGI(s_TAG, "Room record length is %lu and should be %d", this->file.position() - pos,
                         this->header.roomRecordSize);
                return false;
            }
            recs++;
        }
    ...
    while (recs < this->header.shadeRecords) {
        uint32_t pos = this->file.position();
        if (!this->seekChar(CFG_REC_END)) {
            ESP_LOGI(s_TAG, "Failed to find the shade record end %d", recs);
            return false;
        }
        if (this->file.position() - pos != this->header.shadeRecordSize) {
            ESP_LOGI(s_TAG, "Shade record length is %lu and should be %d", this->file.position() - pos,
                     this->header.shadeRecordSize);
            return false;
        }
        recs++;
    }
    if (this->header.version > 10) {
        ...
        while (recs < this->header.groupRecords) {
            uint32_t pos = this->file.position();
            if (!this->seekChar(CFG_REC_END)) {
                ESP_LOGI(s_TAG, "Failed to find the group record end %d", recs);
                return false;
            }
            recs++;
            if (this->file.position() - pos != this->header.groupRecordSize) {
                ESP_LOGI(s_TAG, "Group record length is %lu and should be %d", this->file.position() - pos,
                         this->header.groupRecordSize);
                return false;
            }
        }
    }
    if (this->header.version >= 21) {
        ...
        while (recs < this->header.repeaterRecords) {
            // uint32_t pos = this->file.position();
            if (!this->seekChar(CFG_REC_END)) {
                ESP_LOGI(s_TAG, "Failed to find the repeater record end %d", recs);
            }
            recs++;
        }
    }
...
bool ShadeConfigFile::restoreFile(SomfyShadeController *s, const char *filename, restore_options_t &opts)
{
    bool opened = false;
    if (!this->isOpen()) {
        ESP_LOGI(s_TAG, "Opening shade restore file");
        this->begin(filename, true);
        opened = true;
    }
    if (!this->validate()) {
        ESP_LOGI(s_TAG, "Shade restore file invalid!");
        if (opened) this->end();
        return false;
    }
    if (opts.shades) {
        ...
        for (uint8_t i = 0; i < this->header.shadeRecords; i++) {
            ...
            if (i > 0) ESP_LOGI(s_TAG, ",");
            ESP_LOGI(s_TAG, "%d", s->shades[i].getShadeId());
        }
        ESP_LOGI(s_TAG, "");
        if (this->header.shadeRecords < SOMFY_MAX_SHADES) {
        ...
        ESP_LOGI(s_TAG, "Shade data ignored");
        // FF past the shades and groups.
        this->file.seek(this->file.position() + (this->header.shadeRecords * this->header.shadeRecordSize) +
                            (this->header.groupRecords * this->header.groupRecordSize),
                        SeekSet); // Start at the beginning of the file after the header.
    }
    if (opts.repeaters) {
...
bool ShadeConfigFile::readNetRecord(restore_options_t &opts)
{
    if (this->header.netRecordSize > 0) {
        uint32_t startPos = this->file.position();
        if (opts.network) {
            ESP_LOGI(s_TAG, "Reading network settings from file...");
        ...
        if (opts.network) {
            if (strncmp(settings.serverId, this->header.serverId, sizeof(settings.serverId)) == 0) {
                ESP_LOGI(s_TAG, "Restoring Ethernet adapter settings");
        ...
        if (this->file.position() != startPos + this->header.netRecordSize) {
            ESP_LOGI(s_TAG, "Reading to end of network record");
...
bool ShadeConfigFile::readTransRecord(transceiver_config_t &cfg)
{
    if (this->header.transRecordSize > 0) {
        uint32_t startPos = this->file.position();
        ESP_LOGI(s_TAG, "Reading Transceiver settings from file...");
        ...
        if (this->file.position() != startPos + this->header.transRecordSize) {
            ESP_LOGI(s_TAG, "Reading to end of transceiver record");
            this->seekChar(CFG_REC_END);
        }
    }
    return true;
...
bool ShadeConfigFile::readSettingsRecord()
{
    if (this->header.settingsRecordSize > 0) {
        uint32_t startPos = this->file.position();
        ESP_LOGI(s_TAG, "Reading settings from file...");
        char ver[24];
        ...
        if (this->file.position() != startPos + this->header.settingsRecordSize) {
            ESP_LOGI(s_TAG, "Reading to end of settings record");
            this->seekChar(CFG_REC_END);
        }
    }
    return true;
...
bool ShadeConfigFile::readGroupRecord(SomfyGroup *group)
{
    ...
    if (this->header.version >= 13)
        group->sortOrder = static_cast<int8_t>(this->readUInt8(static_cast<uint8_t>(group->getGroupId() - 1)));
    else
        group->sortOrder = static_cast<int8_t>(group->getGroupId() - 1);
    ...
    if (this->file.position() != startPos + this->header.groupRecordSize) {
        ESP_LOGI(s_TAG, "Reading to end of group record");
        this->seekChar(CFG_REC_END);
    }
    return true;
}
...
bool ShadeConfigFile::readRoomRecord(SomfyRoom *room)
{
    ...
    if (this->file.position() != startPos + this->header.roomRecordSize) {
        ESP_LOGI(s_TAG, "Reading to end of room record");
        this->seekChar(CFG_REC_END);
    }
    return true;
}
...
bool ShadeConfigFile::readShadeRecord(SomfyShade *shade)
{
    ...
    if (this->header.version > 7) shade->flags.setFlags(this->readUInt8(0));
    ...
    if (shade->getShadeId() == 255)
        ...
        shade->p_myPos(100.0f);
        shade->currentPos = shade->target = 100.0f;
    }
    pref.end();
...
bool ShadeConfigFile::loadFile(SomfyShadeController *s, const char *filename)
{
    bool opened = false;
    if (!this->isOpen()) {
        ESP_LOGI(s_TAG, "Opening shade config file");
        this->begin(filename, true);
        opened = true;
    }
    if (!this->validate()) {
        ESP_LOGE(s_TAG, "Shade config file invalid!");
        if (opened) this->end();
        return false;
    }
    for (uint8_t i = 0; i < this->header.roomRecords; i++) {
    ...
    if (opened) {
        ESP_LOGI(s_TAG, "Closing shade config file");
        this->end();
    }
    return true;
}
