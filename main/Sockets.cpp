...
void SocketEmitter::begin()
{
    ...
    ESP_LOGI(s_TAG, "Socket Server Started on port %d", socketsPort);
}
...
{
    this->json.beginEvent(&sockServer, evt, s_response, sizeof(s_response));
    return &this->json;
}
    ...
    for (size_t i = 0; i < sizeof(this->newClients); i++) {
        ...
        if (num != 255) {
            if (sockServer.clientIsConnected(num)) {
                ESP_LOGI(s_TAG, "Initializing Socket Client %u", num);
                ESP_LOGD(s_TAG, "initClients[%u]: emitSockets", num);
                settings.emitSockets(num);
                ESP_LOGD(s_TAG, "initClients[%u]: emitState", num);
                somfy.emitState(num);
                ESP_LOGD(s_TAG, "initClients[%u]: emitUpdateCheck", num);
                git.emitUpdateCheck(num);
                ESP_LOGD(s_TAG, "initClients[%u]: net.emitSockets", num);
                net.emitSockets(num);
                ESP_LOGD(s_TAG, "initClients[%u]: done", num);
            }
...
void SocketEmitter::wsEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
    ...
    case WStype_ERROR:
        if (length > 0)
            ESP_LOGE(s_TAG, "Socket Error: %s", payload);
        else
            ESP_LOGE(s_TAG, "Socket Error: \n");
        break;
    case WStype_DISCONNECTED:
        if (length > 0)
            ESP_LOGI(s_TAG, "WS [%u] Disconnected! [%s]", num, payload);
        else
            ESP_LOGI(s_TAG, "WS [%u] Disconnected!", num);
        for (uint8_t i = 0; i < SOCK_MAX_ROOMS; i++) {
    ...
    case WStype_CONNECTED: {
        ...
        ESP_LOGI(s_TAG, "WS [%u] Connected from %d.%d.%d.%d url: %s", num, ip[0], ip[1], ip[2], ip[3], payload);
    ...
    case WStype_TEXT:
        if (strncmp((char *)payload, "join:", 5) == 0) {
            ...
            ESP_LOGI(s_TAG, "Client %u joining room %u", num, roomNum);
            ...
            ESP_LOGI(s_TAG, "Client %u leaving room %u", num, roomNum);
            if (roomNum < SOCK_MAX_ROOMS) sockEmit.rooms[roomNum].leave(num);
        } else {
            ESP_LOGI(s_TAG, "Socket [%u] text: %s", num, payload);
        }
        ...
        break;
    case WStype_BIN:
        ESP_LOGI(s_TAG, "[%u] get binary length: %u", num, length);
        ...
        break;
    case WStype_PING:
        ESP_LOGI(s_TAG, "WS [%u] Ping received", num);
        break;
    case WStype_PONG:
        ESP_LOGI(s_TAG, "WS [%u] Pong received", num);
        break;
    default:
        ESP_LOGI(s_TAG, "WS [%u] Unhandled Event: %d", num, type);
        break;
    }
}
