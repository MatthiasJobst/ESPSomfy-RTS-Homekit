...
{
    ESP_LOGI(s_TAG, "Checking authentication");
    const bool authNotRequired =
        settings.Security.type == security_types::None ||
        (!cfg && (settings.Security.permissions & static_cast<uint8_t>(security_permissions::ConfigOnly)));
    if (authNotRequired) return true;
    if (server.hasHeader("apikey")) {
        // Api key was supplied.
        ESP_LOGI(s_TAG, "Checking API Key...");
        char token[65];
        ...
        // Send a 401
        ESP_LOGI(s_TAG, "Not authenticated...");
...
{
    ESP_LOGI(s_TAG, "Logging out of webserver");
...
void Web::begin()
{
    ...
    server.on(
        "/restore", HTTP_POST, []() { webServer.handleRestore(server); },
        []() { webServer.handleRestoreUpload(server); });
    server.on("/index.js", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/index.js", "text/javascript");
    });
    server.on("/ui.js", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/ui.js", "text/javascript");
    });
    server.on("/settings.js", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/settings.js", "text/javascript");
    });
    server.on("/somfy.js", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/somfy.js", "text/javascript");
    });
    server.on("/extras.js", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/extras.js", "text/javascript");
    });
    server.on("/qrcode.min.js", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/qrcode.min.js", "text/javascript");
    });
    server.on("/main.css", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/main.css", "text/css");
    });
    server.on("/widgets.css", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/widgets.css", "text/css");
    });
    server.on("/icons.css", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/icons.css", "text/css");
    });
    server.on("/favicon.png", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/favicon.png", "image/png");
    });
    server.on("/icon.png", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/icon.png", "image/png");
    });
    server.on("/icon.svg", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/icon.svg", "image/svg+xml");
    });
    server.on("/apple-icon.png", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/apple-icon.png", "image/png");
    });
    server.on("/HomeKit.svg", []() {
        webServer.sendCacheHeaders(604800);
        webServer.handleStreamFile(server, "/HomeKit.svg", "image/svg+xml");
    });
    ...
    server.on(
        "/updateFirmware", HTTP_POST, []() { webServer.handleUpdateFirmware(server); },
        []() { webServer.handleUpdateFirmwareUpload(server); });
    server.on(
        "/updateShadeConfig", HTTP_POST, []() { webServer.handleUpdateShadeConfig(server); },
        []() { webServer.handleUpdateShadeConfigUpload(server); });
    server.on(
        "/updateApplication", HTTP_POST, []() { webServer.handleUpdateApplication(server); },
