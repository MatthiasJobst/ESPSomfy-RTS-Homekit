...
void SomfyMovementTracker::checkMovement() {
  ...
  if(!tilt_first && shade->direction > 0) {
    if(downTime == 0) {
      ...
      int32_t msFrom0 = (int32_t)floor((shade->startPos/100) * downTime);
      msFrom0 += (curTime - shade->moveStart);
    ...
    if(shade->currentPos >= shade->target) {
      ...
      if(shade->motionState.settingPos) {
        if(!shade->isAtTarget()) {
    ...
    if(upTime == 0) {
      ...
      int32_t msFrom100 = upTime - (int32_t)floor((shade->startPos/100) * upTime);
      msFrom100 += (curTime - shade->moveStart);
  ...
  if(shade->tiltDirection > 0) {
    if(tilt_first) shade->moveStart = curTime;
    int32_t msFrom0 = (int32_t)floor((shade->startTiltPos/100) * tiltTime);
    msFrom0 += (curTime - shade->tiltStart);
    ...
    if(tilt_first) {
      ...
      if(shade->motionState.settingTiltPos) {
      ...
      shade->motionState.settingTiltPos = false;
      if(shade->isAtTarget()) shade->commitShadePosition();
    }
  }
    ...
    if(tiltTime == 0) {
      ...
      int32_t msFrom100 = tiltTime - (int32_t)floor((shade->startTiltPos/100) * tiltTime);
      msFrom100 += (curTime - shade->tiltStart);
  ...
  if(shade->motionState.settingMyPos && shade->isAtTarget()) {
    delay(200);
    if(shade->tiltType != tilt_types::none) {
      if(shade->myTiltPos == shade->currentTiltPos && shade->myPos == shade->currentPos)
        shade->myPos = shade->myTiltPos = -1;
      else {
...
void SomfyMovementTracker::setMovement(int8_t dir) {
  ...
  if(dir == 0) {
    ...
    shade->tiltStart = shade->moveStart = millis();
    shade->startPos     = shade->currentPos;
    shade->startTiltPos = shade->currentTiltPos;
  }
}
