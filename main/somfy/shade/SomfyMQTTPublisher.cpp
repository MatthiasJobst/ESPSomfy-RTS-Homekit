...
{
    if (mqtt.connected()) {
        snprintf(s_mqttTopicBuffer, sizeof(s_mqttTopicBuffer), "shades/%u/%s", shade->getShadeId(), topic);
...
void SomfyMQTTPublisher::unpublish()
{
    SomfyMQTTPublisher::unpublish(shade->getShadeId());
}
...
{
    if (mqtt.connected()) {
        snprintf(s_mqttTopicBuffer, sizeof(s_mqttTopicBuffer), "shades/%u/%s", id, topic);
...
void SomfyMQTTPublisher::emitCommand(uint8_t num, somfy_commands cmd, const char *source, uint32_t sourceAddress,
                                     const char *evt)
{
    JsonSockEvent *json = sockEmit.beginEmit(evt);
    json->beginObject();
