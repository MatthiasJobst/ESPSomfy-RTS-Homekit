  ...
  if(shade->lastFrame.await > 0 && (millis() > shade->lastFrame.await)) {
    ...
    switch(cmd) {
      ...
      case somfy_commands::Up:
        if(shade->tiltType == tilt_types::tiltmotor) { // Theoretically this should get here unless it does have a tilt motor.
          if(shade->lastFrame.repeats >= TILT_REPEATS) {
            ...
            shade->lastFrame.processed = true;
            ESP_LOGI(TAG, "%s Processing tilt %s after %d repeats", shade->name, translateSomfyCommand(shade->lastFrame.cmd).c_str(), shade->lastFrame.repeats);
            shade->emitCommand(cmd, "remote", shade->lastFrame.remoteAddress);
          }
          else {
            int8_t dir = shade->lastFrame.cmd == somfy_commands::Up ? -1 : 1;
          ...
          if(shade->lastFrame.repeats >= TILT_REPEATS) {
            ...
            shade->setMovement(dir);
            shade->lastFrame.processed = true;
            ESP_LOGI(TAG, "%s Processing %s after %d repeats", shade->name, translateSomfyCommand(shade->lastFrame.cmd).c_str(), shade->lastFrame.repeats);
            shade->emitCommand(cmd, "remote", shade->lastFrame.remoteAddress);
          }
          else {
            int8_t dir = shade->lastFrame.cmd == somfy_commands::Up ? -1 : 1;
      ...
      case somfy_commands::My:
        if(shade->lastFrame.repeats >= SETMY_REPEATS && shade->isIdle()) {
          if(floor(shade->myPos) == floor(shade->currentPos)) {
            // We are clearing it.
            shade->p_myPos(-1);
            shade->p_myTiltPos(-1);
          }
          ...
          if(shade->simMy())
            ...
            if(shade->myPos >= 0.0f && shade->myPos <= 100.0f) shade->p_target(shade->myPos);
        ...
        if(shade->lastFrame.processed) {
          ESP_LOGI(TAG, "%s Processing MY after %d repeats", shade->name, shade->lastFrame.repeats);
        }
        break;
      default:
        break;
  ...
  if(!(shade->flags & static_cast<uint8_t>(somfy_flags_t::Windy))) {
    ...
    if(isSunny && shade->flagManager.sunDone) {
      if(shade->tiltType == tilt_types::tiltonly)
        shade->p_tiltTarget(shade->myTiltPos >= 0 ? shade->myTiltPos : 100.0f);
      else
        shade->p_target(shade->myPos >= 0 ? shade->myPos : 100.0f);
    }
...
void SomfyCommandProcessor::processMyCommand(bool internal, somfy_frame_t &frame, uint64_t curTime) {
  ...
  if(shade->isIdle()) {
    if(!internal) {
      ...
      shade->lastFrame.processed = true;
      if(shade->myTiltPos >= 0.0f && shade->myTiltPos <= 100.0f) shade->p_tiltTarget(shade->myTiltPos);
      if(shade->myPos >= 0.0f && shade->myPos <= 100.0f && shade->tiltType != tilt_types::tiltonly) shade->p_target(shade->myPos);
      shade->emitCommand(cmd, internal ? "internal" : "remote", frame.remoteAddress);
    }
  }
  else {
...
void SomfyCommandProcessor::processUpDownCommand(somfy_commands cmd, int8_t moveDir, bool internal, somfy_frame_t &frame, uint64_t curTime) {
  ...
  if(moveDir > 0 && shade->flagManager.windLast && (curTime - shade->flagManager.windLast) < SOMFY_NO_WIND_REMOTE_TIMEOUT) return;

  if(shade->tiltType == tilt_types::tiltmotor || shade->tiltType == tilt_types::euromode) {
    if(!internal) shade->lastFrame.await = curTime + 500;
    else          shade->lastFrame.processed = true;
    ...
    if(!internal) {
      if(moveDir < 0) {
        ...
        if(shade->tiltType == tilt_types::tiltonly) shade->p_tiltTarget(endpoint);
        else { shade->p_target(endpoint); shade->p_tiltTarget(endpoint); }
      }
      else {
...
void SomfyCommandProcessor::processFrame(somfy_frame_t &frame, bool internal) {
  ...
  int8_t dir = 0;
  shade->moveStart = shade->tiltStart = curTime;
  shade->startPos = shade->currentPos;
  shade->startTiltPos = shade->currentTiltPos;
  if(!internal) shade->motionState.clear();
...
void SomfyCommandProcessor::processInternalCommand(somfy_commands cmd, uint8_t repeat) {
  ...
  switch(cmd) {
    ...
    case somfy_commands::My:
      if(shade->isIdle()) {
        ESP_LOGI(TAG, "Shade #%d is idle", shade->getShadeId());
        if(shade->simMy()) {
          shade->moveToMyPosition();
        }
        else {
        ...
        else shade->p_target(shade->currentPos);
        shade->p_tiltTarget(shade->currentTiltPos);
      }
      break;
    case somfy_commands::StepUp:
    ...
    case somfy_commands::Flag:
      ...
      if(shade->hasSunSensor()) {
        ...
        ESP_LOGI(TAG, "Shade does not have sensor %d", shade->flags);
      }
      break;
    case somfy_commands::SunFlag:
      if(shade->hasSunSensor()) {
