...
class SomfyGPIOControl {
  public:
    ...
    void setGPIOs(radio_proto proto, float currentPos, int8_t direction, int8_t tiltDirection, shade_types shadeType,
                  tilt_types tiltType);

    // Pulse GP_Remote output pins for the command in frame.
    void triggerGPIOs(somfy_frame_t &frame, radio_proto proto, shade_types shadeType, bool isToggle);

    // Return true if this shade is using the given pin number.
    bool usesPin(uint8_t pin, radio_proto proto, shade_types shadeType, bool isToggle) const;
};
