...
static void logTimerDone(uint8_t shadeId, const char *label) {
    ESP_LOGI(TAG, "[%u] %s -> done", shadeId, label);
}

static void logTimerStart(uint8_t shadeId, const char *label) {
    ESP_LOGI(TAG, "[%u] %s -> start", shadeId, label);
}
...
void SomfyFlagManager::updateTimers(bool wasSunny, bool wasWindy,
    ...
    if(isSunny && !wasSunny)        { this->sunStart   = curTime; this->sunDone   = false; logTimerStart(shadeId, "Sun"); }
    else if(!isSunny && wasSunny)   { this->noSunStart = curTime; this->noSunDone = false; logTimerStart(shadeId, "No Sun"); }
    if(isWindy && !wasWindy)        { this->windStart   = curTime; this->windDone   = false; logTimerStart(shadeId, "Wind"); }
    else if(!isWindy && wasWindy)   { this->noWindStart = curTime; this->noWindDone = false; logTimerStart(shadeId, "No Wind"); }
}
