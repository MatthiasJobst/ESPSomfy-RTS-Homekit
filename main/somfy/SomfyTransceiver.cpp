...
// Module-local state: rxmode, bit_length, s_somfy_rx, s_rx_queue, s_tx_queue, freq scan vars.
...
static const char *s_TAG = "SomfyTransceiver";
...
static int s_interruptPin = 0;

// RF noise watchdog state (written in ISR, read in loop)
static volatile bool s_noiseDetected = false;
static volatile uint32_t s_noiseWindowStart = 0;
static volatile uint16_t s_noisePulseCount = 0;
static const uint16_t s_NOISE_PULSE_THRESHOLD = 100;
static const uint32_t s_NOISE_WINDOW_MS = 10000;
static const uint32_t s_NOISE_COOLDOWN_MS = 30000;
...
static volatile struct {
    ...
    uint32_t glitchCalls;   // incremented for every pulse < s_bitMin
    ...
    uint8_t status;         // s_somfy_rx.status at last valid-pulse ISR call
    bool waitingHalf;       // s_somfy_rx.waiting_half_symbol at last valid-pulse ISR call
    bool frameQueued;       // set when a complete frame is pushed to s_rx_queue
} s_rxDbg;
...
static const uint32_t s_tempo_wakeup_min = static_cast<uint32_t>(9415 * TOLERANCE_MIN);
static const uint32_t s_tempo_wakeup_max = static_cast<uint32_t>(9415 * TOLERANCE_MAX);
...
static const uint32_t s_tempo_synchro_hw_min = SYMBOL * 4 * TOLERANCE_MIN;
static const uint32_t s_tempo_synchro_hw_max = SYMBOL * 4 * TOLERANCE_MAX;
static const uint32_t s_tempo_synchro_sw_min = 4850 * TOLERANCE_MIN;
static const uint32_t s_tempo_synchro_sw_max = 4850 * TOLERANCE_MAX;
static const uint32_t s_tempo_half_symbol_min = SYMBOL * TOLERANCE_MIN;
static const uint32_t s_tempo_half_symbol_max = SYMBOL * TOLERANCE_MAX;
static const uint32_t s_tempo_symbol_min = SYMBOL * 2 * TOLERANCE_MIN;
static const uint32_t s_tempo_symbol_max = SYMBOL * 2 * TOLERANCE_MAX;
// static const uint32_t tempo_if_gap = 30415;  // Gap between frames
static int16_t s_bitMin = SYMBOL * TOLERANCE_MIN;
static somfy_rx_t s_somfy_rx;
static somfy_rx_queue_t s_rx_queue;
static somfy_tx_queue_t s_tx_queue;
...
#define SOMFY_WRITE_SYM(e, chan, sym, enc_state, ret)                                                                  \
    do {                                                                                                               \
        ...
        (ret) += (e)->copy_enc->encode((e)->copy_enc, (chan), &(sym), sizeof(rmt_symbol_word_t), &_s);                 \
        if (_s & RMT_ENCODING_MEM_FULL) {                                                                              \
            *(enc_state) = static_cast<rmt_encode_state_t>(static_cast<int>(*(enc_state)) |                            \
                                                           static_cast<int>(RMT_ENCODING_MEM_FULL));                   \
            return (ret);                                                                                              \
        }                                                                                                              \
    } while (0)

static size_t somfy_encode(rmt_encoder_t *enc, rmt_channel_handle_t chan, const void *data, size_t data_size,
                           rmt_encode_state_t *ret_state)
{
    ...
    while (e->state != ST_DONE) {
        ...
        case ST_SYNC:
            if (e->idx < f->sync_count) {
                sym.level0 = 1;
                sym.duration0 = 4 * SYMBOL;
                sym.level1 = 0;
                sym.duration1 = 4 * SYMBOL;
                SOMFY_WRITE_SYM(e, chan, sym, ret_state, n);
                e->idx++;
            } else {
                e->state = ST_SWSYNC;
    ...
    *ret_state =
        static_cast<rmt_encode_state_t>(static_cast<int>(*ret_state) | static_cast<int>(RMT_ENCODING_COMPLETE));
    return n;
}
...
void IRAM_ATTR SomfyTransceiver::handleReceiveISR(void *)
{
    SomfyTransceiver::handleReceive();
}
...
void RECEIVE_ATTR SomfyTransceiver::handleReceive()
{
    ...
    if (somfy.transceiver.config.noiseDetection) {
        uint32_t now = millis();
        if (now - s_noiseWindowStart > s_NOISE_WINDOW_MS) {
            s_noiseWindowStart = now;
            s_noisePulseCount = 0;
        }
        s_noisePulseCount = s_noisePulseCount + 1;
        if (s_noisePulseCount > s_NOISE_PULSE_THRESHOLD) {
            gpio_intr_disable((gpio_num_t)s_interruptPin);
            s_noiseDetected = true;
            s_noisePulseCount = 0;
            return;
        }
    }

    if (duration < s_bitMin) {
        ...
        if (s_somfy_rx.pulseCount < MAX_TIMINGS && s_somfy_rx.cpt_synchro_hw > 0)
            s_somfy_rx.pulses[s_somfy_rx.pulseCount++] = duration;
        s_rxDbg.glitchCalls = s_rxDbg.glitchCalls + 1;
        return;
    }
    s_rxDbg.lastDuration = duration;
    s_rxDbg.hwSyncCount = s_somfy_rx.cpt_synchro_hw;
    s_rxDbg.bitsReceived = s_somfy_rx.cpt_bits;
    s_rxDbg.status = s_somfy_rx.status;
    s_rxDbg.waitingHalf = s_somfy_rx.waiting_half_symbol;
    last_time = time;
    switch (s_somfy_rx.status) {
    case waiting_synchro:
        if (s_somfy_rx.pulseCount < MAX_TIMINGS) s_somfy_rx.pulses[s_somfy_rx.pulseCount++] = duration;
        if (duration > s_tempo_synchro_hw_min && duration < s_tempo_synchro_hw_max) {
            ...
            memset(s_somfy_rx.payload, 0x00, sizeof(s_somfy_rx.payload));
            s_somfy_rx.previous_bit = 0x00;
            s_somfy_rx.waiting_half_symbol = false;
            s_somfy_rx.cpt_bits = 0;
            // 80-bit frames give 12, 13 (RS485 device) or 24+ hw-sync bits.
            // 56-bit frames give 4 or 14. Everything else falls back to 56.
            const uint8_t hw = s_somfy_rx.cpt_synchro_hw;
            s_somfy_rx.bit_length = (hw == 12 || hw == 13 || hw > 17) ? 80 : 56;
            // s_somfy_rx.bit_length = 80;
            s_somfy_rx.status = receiving_data;
        } else {
            // Reset and start looking for hardware sync again.
            s_somfy_rx.cpt_synchro_hw = 0;
            // Try to capture the wakeup pulse.
            if (duration > s_tempo_wakeup_min && duration < s_tempo_wakeup_max) {
                memset(&s_somfy_rx.payload, 0x00, sizeof(s_somfy_rx.payload));
                s_somfy_rx.previous_bit = 0x00;
                s_somfy_rx.waiting_half_symbol = false;
                s_somfy_rx.cpt_bits = 0;
                s_somfy_rx.bit_length = 56;
            } else if ((s_somfy_rx.pulseCount > 20 && s_somfy_rx.cpt_synchro_hw == 0) || duration > 250000) {
                s_somfy_rx.pulseCount = 0;
            }
        }
        break;
    case receiving_data:
        ...
        if (duration > s_tempo_symbol_min && duration < s_tempo_symbol_max && !s_somfy_rx.waiting_half_symbol) {
            s_somfy_rx.previous_bit = 1 - s_somfy_rx.previous_bit;
            // Bits come in high order bit first.
            s_somfy_rx.payload[s_somfy_rx.cpt_bits / 8] += s_somfy_rx.previous_bit << (7 - s_somfy_rx.cpt_bits % 8);
            ++s_somfy_rx.cpt_bits;
        } else if (duration > s_tempo_half_symbol_min && duration < s_tempo_half_symbol_max) {
            if (s_somfy_rx.waiting_half_symbol) {
                s_somfy_rx.waiting_half_symbol = false;
                s_somfy_rx.payload[s_somfy_rx.cpt_bits / 8] += s_somfy_rx.previous_bit << (7 - s_somfy_rx.cpt_bits % 8);
                ++s_somfy_rx.cpt_bits;
            } else {
                s_somfy_rx.waiting_half_symbol = true;
            }
        } else {
            //++s_somfy_rx.cpt_bits;
            ...
            memset(&s_somfy_rx.payload, 0x00, sizeof(s_somfy_rx.payload));
            s_somfy_rx.pulseCount = 1;
            s_somfy_rx.cpt_synchro_hw = 0;
            s_somfy_rx.previous_bit = 0x00;
            s_somfy_rx.waiting_half_symbol = false;
            s_somfy_rx.cpt_bits = 0;
            s_somfy_rx.bit_length = 56;
            s_somfy_rx.status = waiting_synchro;
            s_somfy_rx.pulses[0] = duration;
        }
        break;
    default:
        break; // LCOV_EXCL_LINE
    }
    if (s_somfy_rx.status == receiving_data && s_somfy_rx.cpt_bits >= s_somfy_rx.bit_length) {
        ...
        if (s_rx_queue.length >= MAX_RX_BUFFER) {
            // We have overflowed the buffer simply empty the last item
            // in this instance we will simply throw it away.
            uint8_t ndx = s_rx_queue.index[MAX_RX_BUFFER - 1];
            if (ndx < MAX_RX_BUFFER) s_rx_queue.items[ndx].pulseCount = 0;
            // memset(&this->items[ndx], 0x00, sizeof(somfy_rx_t));
            s_rx_queue.index[MAX_RX_BUFFER - 1] = 255;
            s_rx_queue.length--;
        }
        uint8_t first = 0;
        ...
        for (uint8_t i = 0; i < MAX_RX_BUFFER; i++) {
            if (s_rx_queue.items[i].pulseCount == 0) {
                first = i;
                memcpy(&s_rx_queue.items[i], &s_somfy_rx, sizeof(somfy_rx_t));
        ...
        for (uint8_t i = MAX_RX_BUFFER - 1; i > 0; i--) {
            s_rx_queue.index[i] = s_rx_queue.index[i - 1];
        }
        s_rx_queue.length++;
        s_rx_queue.index[0] = first;
        memset(&s_somfy_rx.payload, 0x00, sizeof(s_somfy_rx.payload));
        s_somfy_rx.cpt_synchro_hw = 0;
        s_somfy_rx.previous_bit = 0x00;
        s_somfy_rx.waiting_half_symbol = false;
...
void SomfyTransceiver::beginFrequencyScan()
{
    if (this->config.enabled) {
        ...
        s_interruptPin = this->config.RXPin;
        ELECHOUSE_cc1101.setRxBW(
            this->config
                .rxBandwidth); // Set the Receive Bandwidth in kHz. Value from 58.03 to 812.50. Default is 812.50 kHz.
        ELECHOUSE_cc1101.SetRx();
        markFreq = currFreq = 433.0f;
        markRSSI = -100;
        ELECHOUSE_cc1101.setMHZ(currFreq);
        ESP_LOGI(s_TAG, "Begin frequency scan on Pin #%d", this->config.RXPin);
        gpio_set_intr_type((gpio_num_t)s_interruptPin, GPIO_INTR_ANYEDGE);
...
void SomfyTransceiver::processFrequencyScan(bool received)
{
    if (this->config.enabled && rxmode == 3) {
        ...
        if (millis() - lastScan > 100 && s_somfy_rx.status == waiting_synchro) {
            lastScan = millis();
            this->emitFrequencyScan();
            currFreq += 0.01f;
...
void SomfyTransceiver::endFrequencyScan()
{
    if (rxmode == 3) {
        rxmode = 0;
        if (s_interruptPin > 0) gpio_isr_handler_remove((gpio_num_t)s_interruptPin);
        s_interruptPin = 0;
        this->config.apply();
        this->emitFrequencyScan();
    }
...
void SomfyTransceiver::emitFrequencyScan(uint8_t num)
{
    ...
    /*
    char buf[420];
    snprintf(buf, sizeof(buf), "{\"scanning\":%s,\"testFreq\":%f,\"testRSSI\":%d,\"frequency\":%f,\"RSSI\":%d}", rxmode
    == 3 ? "true" : "false", currFreq, currRSSI, markFreq, markRSSI); if(num >= 255)
    sockEmit.sendToClients("frequencyScan", buf); else sockEmit.sendToClient(num, "frequencyScan", buf);
    */
}
...
bool SomfyTransceiver::receive(somfy_rx_t *rx)
{
    ...
    if (s_rx_queue.length > 0) {
        ESP_LOGD(s_TAG, "Processing receive %d", s_rx_queue.length);
        s_rx_queue.pop(rx);
        this->frame.decodeFrame(rx);
        ESP_LOGI(s_TAG, "Frame decoded: addr=0x%06" PRIx32 " rcode=%u cmd=%s valid=%d bits=%u",
                 (uint32_t)this->frame.remoteAddress, (unsigned)this->frame.rollingCode,
                 translateSomfyCommand(this->frame.cmd).c_str(), (int)this->frame.valid, (unsigned)rx->bit_length);
...
void SomfyTransceiver::enableReceive(void)
{
    ...
    if (this->config.enabled) {
        ...
        gpio_isr_handler_add((gpio_num_t)s_interruptPin, SomfyTransceiver::handleReceiveISR, NULL);
        ESP_LOGD(s_TAG, "Enabled receive on Pin #%d Timing: %ld", this->config.RXPin, millis() - timing);
    }
}
...
{
    if (this->config.enabled) {
        if (this->config.SCKPin == pin || this->config.TXPin == pin || this->config.RXPin == pin ||
            this->config.MOSIPin == pin || this->config.MISOPin == pin || this->config.CSNPin == pin)
            return true;
    }
    return false;
}
...
{
    ESP_LOGD(s_TAG, "Deserialize Radio JSON Config");
...
{
    if (pref.isKey(key)) {
        ESP_LOGD(s_TAG, "Removing NVS Key: CC1101.%s", key);
        pref.remove(key);
    }
}
...
void transceiver_config_t::load()
{
    ...
    case esp_chip_model_t::CHIP_ESP32S3:
        ESP_LOGD(s_TAG, "Setting S3 Transceiver Defaults...");
        this->TXPin = 15;
        this->RXPin = 14;
        this->MOSIPin = 11;
        this->MISOPin = 13;
...
void transceiver_config_t::apply()
{
    ...
    if (this->enabled) {
        ...
        pref.end();
        if (!radioInit) return;
        ESP_LOGD(s_TAG, "Applying radio settings ");
        ESP_LOGD(s_TAG, "Setting Data Pins RX:%u TX:%u", this->RXPin, this->TXPin);
        ...
        ESP_LOGD(s_TAG, "Setting SPI Pins SCK:%u MISO:%u MOSI:%u CSN:%u", this->SCKPin, this->MISOPin, this->MOSIPin,
                 this->CSNPin);
        ESP_LOGD(s_TAG, "Radio Pins Configured!");
        ELECHOUSE_cc1101.Init();
        ELECHOUSE_cc1101.setCCMode(0);            // set config for internal transmission mode.
        ELECHOUSE_cc1101.setMHZ(this->frequency); // Here you can set your basic frequency. The lib calculates the
                                                  // frequency automatically (default = 433.92).The cc1101 can: 300-348
                                                  // MHZ, 387-464MHZ and 779-928MHZ. Read More info from datasheet.
        ELECHOUSE_cc1101.setRxBW(
            this->rxBandwidth); // Set the Receive Bandwidth in kHz. Value from 58.03 to 812.50. Default is 812.50 kHz.
        ELECHOUSE_cc1101.setDeviation(
            this->deviation); // Set the Frequency deviation in kHz. Value from 1.58 to 380.85. Default is 47.60 kHz.
        ELECHOUSE_cc1101.setPA(
            this->txPower); // Set TxPower. The following settings are possible depending on the frequency band.  (-30
        ...
        ELECHOUSE_cc1101.setCrc(
            0); // 1 = CRC calculation in TX and CRC check in RX enabled. 0 = CRC disabled for TX and RX.
        ELECHOUSE_cc1101.setCRC_AF(
            0); // Enable automatic flush of RX FIFO when CRC is not OK. This requires that only one packet is in the
                // RXIFIFO and that packet length is limited to the RX FIFO size.
        ...
        if (!ELECHOUSE_cc1101.getCC1101()) {
            ESP_LOGE(s_TAG, "Error setting up the radio");
            this->radioInit = false;
        } else {
            ESP_LOGI(s_TAG, "Successfully set up the radio");
...
bool SomfyTransceiver::begin()
{
    ...
    this->config.apply();
    s_rx_queue.init();
    ESP_LOGD(s_TAG, "RMT build: transceiver enabled=%d TXPin=%d", this->config.enabled, this->config.TXPin);
    if (this->config.enabled && s_rmtTxChan == nullptr) {
        ...
        ESP_LOGD(s_TAG, "RMT TX channel initialised on GPIO %d", this->config.TXPin);
...
void SomfyTransceiver::loop()
{
    ...
    if (s_noiseDetected) {
        ...
        if (noiseDisabledAt == 0) {
            ...
            ESP_LOGW(s_TAG, "RF noise detected — RX disabled for %lu ms", (unsigned long)s_NOISE_COOLDOWN_MS);
        } else if (millis() - noiseDisabledAt > s_NOISE_COOLDOWN_MS) {
            ESP_LOGD(s_TAG, "RF noise cooldown elapsed — re-enabling RX");
            s_noiseDetected = false;
            noiseDisabledAt = 0;
            s_noisePulseCount = 0;
            s_noiseWindowStart = millis();
            gpio_intr_enable((gpio_num_t)s_interruptPin);
        }
        return;
    }
    ...
    if (millis() - lastTelemAt > 5000) {
        ...
        ESP_LOGD(s_TAG,
                 "ISR telem: total=%lu (+%lu) glitch=%lu reset=%lu lastDur=%lu µs resetDur=%lu µs hwSync=%u bits=%u "
                 "waitHalf=%d status=%u",
                 (unsigned long)calls, (unsigned long)(calls - lastTotalCalls), (unsigned long)s_rxDbg.glitchCalls,
                 (unsigned long)s_rxDbg.resetCalls, (unsigned long)s_rxDbg.lastDuration,
                 (unsigned long)s_rxDbg.resetDuration, (unsigned)s_rxDbg.hwSyncCount, (unsigned)s_rxDbg.bitsReceived,
                 (int)s_rxDbg.waitingHalf, (unsigned)s_rxDbg.status);
        lastTotalCalls = calls;
    }
    if (s_rxDbg.frameQueued) {
        s_rxDbg.frameQueued = false;
        ESP_LOGD(s_TAG, "ISR queued complete frame");
    }
    ...
    if (rxmode == 3) {
        ...
        if (s_rmtBusy && s_txDone) {
            ESP_LOGD(s_TAG, "RMT TX done — calling endTransmit");
            this->endTransmit();
            s_rmtBusy = false;
            s_txDone = false;
            s_busyWarnAt = 0;
            s_tx_queue.delay_time = millis() + TX_QUEUE_DELAY;
        } else if (s_rmtBusy) {
            if (s_busyWarnAt == 0)
                s_busyWarnAt = millis();
            else if (millis() - s_busyWarnAt > 3000) {
                ESP_LOGW(s_TAG, "RMT busy for >3s — txDone=%d (ISR never fired?)", (int)s_txDone);
                s_busyWarnAt = millis();
            }
        } else {
            s_busyWarnAt = 0;
        ...
        // Drain one repeater frame from the s_tx_queue when the channel is free.
        if (s_tx_queue.length > 0 && millis() > s_tx_queue.delay_time && s_somfy_rx.cpt_synchro_hw == 0 && !s_rmtBusy) {
            somfy_tx_t tx;
            s_tx_queue.pop(&tx);
            ESP_LOGD(s_TAG, "Sending frame %d - %d-BIT [", tx.hwsync, tx.bit_length);
            for (uint8_t j = 0; j < 10; j++) {
                ESP_LOGD(s_TAG, "%d", tx.payload[j]);
                if (j < 9) ESP_LOGD(s_TAG, ", ");
            }
            ESP_LOGD(s_TAG, "]");
            this->beginTransmit();
...
void SomfyTransceiver::beginFrameTx(somfy_frame_t &frame, uint8_t repeats)
{
    if (repeats >= SOMFY_BURST_MAX) {
        ESP_LOGW(s_TAG, "Capping repeats %u→%u (burst queue depth)", repeats, SOMFY_BURST_MAX - 1);
    ...
    for (uint8_t i = 1; i < total; i++) {
        ...
        if (frame.bitLength == 80) frame.encode80BitFrame(s_burst[i].bytes, i);
    ...
    ESP_LOGD(s_TAG, "RMT beginFrameTx: bitlen=%d frames=%d", frame.bitLength, total);

    uint8_t queued = 0;
    for (uint8_t i = 0; i < total; i++) {
        esp_err_t err = rmt_transmit(s_rmtTxChan, &s_enc->base, &s_burst[i], sizeof(somfy_tx_frame_t), &tx_cfg);
        if (err != ESP_OK) {
            ESP_LOGE(s_TAG, "rmt_transmit frame %u/%u failed: %s", i + 1, total, esp_err_to_name(err));
            break;
        }
        queued++;
    }
    ...
    if (queued < total) {
        ...
        if (s_pendingFrames > skipped)
            s_pendingFrames -= skipped;
        else {
            s_pendingFrames = 0;
            s_txDone = true;
        }
    ...
    ESP_LOGD(s_TAG, "RMT beginFrameTx: queued %u/%u", queued, total);
}
...
void SomfyTransceiver::beginRawFrameTx(byte *payload, uint8_t sync, uint8_t bitLength)
{
    ...
    esp_err_t err = rmt_transmit(s_rmtTxChan, &s_enc->base, &s_burst[0], sizeof(somfy_tx_frame_t), &tx_cfg);
    if (err != ESP_OK) {
        ESP_LOGE(s_TAG, "rmt_transmit raw failed: %s", esp_err_to_name(err));
        s_pendingFrames = 0;
        s_rmtBusy = false;
    }
}
