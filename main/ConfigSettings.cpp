...
bool BaseSettings::loadFile(const char *filename)
{
    ...
    if (LittleFS.exists(filename)) {
        ...
        filesize += file.size();
        ESP_LOGI(s_TAG, "Loading file: %s, size: %u", filename, filesize);
...
bool ConfigSettings::begin()
{
    ...
    ESP_LOGI(s_TAG, "Chip Model ESP32-%s", this->chipModel);
    this->fwVersion.parse(FW_VERSION);
    uint64_t mac = ESP.getEfuseMac();
    for (int i = 0; i < 17; i = i + 8) {
        chipId |= ((mac >> (40 - i)) & 0xff) << i;
    }
    snprintf_P(this->serverId, sizeof(this->serverId), "%02X%02X%02X", (uint16_t)((chipId >> 16) & 0xff),
...
{
    return this->Security.type != security_types::None;
}
...
{
    this->Security.print();
    ESP_LOGI(s_TAG, "Connection Type: %u", (unsigned int)this->connType);
    this->NTP.print();
    if (this->connType == conn_types_t::wifi || this->connType == conn_types_t::unset) this->WIFI.print();
    if (this->connType == conn_types_t::ethernet || this->connType == conn_types_t::ethernetpref)
        this->Ethernet.print();
}
...
{
    return strlen(this->fwVersion.name) + 3 + strlen(this->hostname) + 3 + strlen(this->NTP.ntpServer) + 3 +
           strlen(this->NTP.posixZone) + 3 + 6; // updateCheck
}
uint16_t ConfigSettings::calcNetRecSize()
{
    return 4   // connType
           + 6 // dhcp
           + this->IP.ip.toString().length() + 3 + this->IP.gateway.toString().length() + 3 +
           this->IP.subnet.toString().length() + 3 + this->IP.dns1.toString().length() + 3 +
           this->IP.dns2.toString().length() + 3 + strlen(this->MQTT.protocol) + 3 + strlen(this->MQTT.hostname) + 3 +
           6                                                                          // MQTT Port
           + 6                                                                        // PubDisco
           + strlen(this->MQTT.rootTopic) + 3 + strlen(this->MQTT.discoTopic) + 3 + 4 // ETH.boardType
           + 4                                                                        // ETH.phyType
           + 4                                                                        // ETH.clkMode
           + 5                                                                        // ETH.phyAddress
           + 5                                                                        // ETH.PWRPin
...
{
    ESP_LOGI(s_TAG, "NTP Settings ");
    ESP_LOGI(s_TAG, "NTP Server: %s", this->ntpServer);
    ESP_LOGI(s_TAG, "TZ: %s", this->posixZone);
}
...
bool IPSettings::load()
{
    ...
    ESP_LOGI(s_TAG, "Preference IP Free Entries: %d", pref.freeEntries());
    pref.end();
    return true;
}
...
bool SecuritySettings::load()
{
    ...
    if (pref.isKey("permissions"))
        this->permissions = static_cast<uint8_t>(pref.getChar("permissions", static_cast<int8_t>(this->permissions)));
    pref.end();
    return true;
}
void SecuritySettings::print()
{
    ESP_LOGI(s_TAG, "SECURITY   Type:%u Username:[%s] Password:[%s] Pin:[%s] Permissions:%u",
...
{
    ESP_LOGI(s_TAG, "WIFI Settings");
    ESP_LOGI(s_TAG, " SSID: [%s] PassPhrase: [%s]", this->ssid, this->passphrase);
}
void WifiSettings::printNetworks()
{
    ...
    String network;
    for (int i = 0; i < n; i++) {
        if (WiFi.SSID(i).compareTo(this->ssid) == 0)
            ESP_LOGI(s_TAG, "*");
        else
            ESP_LOGI(s_TAG, " ");
        ESP_LOGI(s_TAG, "%d: %s (", i, WiFi.SSID(i).c_str());
        ESP_LOGI(s_TAG, "RSSI: %d dBm", WiFi.RSSI(i));
        ESP_LOGI(s_TAG, "CH: %d", WiFi.channel(i));
        ESP_LOGI(s_TAG, "MAC: %s", WiFi.BSSIDstr(i).c_str());
    }
}
...
{
    ESP_LOGI(s_TAG, "Ethernet Settings");
    ESP_LOGI(s_TAG, "Board:%d PHYType:%d CLK:%d ADDR:%d PWR:%d MDC:%d MDIO:%d", this->boardType, this->phyType,
             this->CLKMode, this->phyAddress, this->PWRPin, this->MDCPin, this->MDIOPin);
}
void ConfigSettings::printAvailHeap()
{
    ESP_LOGI(s_TAG, "Max Heap: %d", ESP.getMaxAllocHeap());
    ESP_LOGI(s_TAG, "Free Heap: %d", ESP.getFreeHeap());
    ESP_LOGI(s_TAG, "Min Heap: %d", ESP.getMinFreeHeap());
}
