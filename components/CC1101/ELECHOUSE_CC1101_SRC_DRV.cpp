...
void ELECHOUSE_CC1101::SpiStart(void)
{
  ...
  // enable SPI
  // Pass -1 for SS so only SCK/MISO/MOSI are routed through the SPI peripheral.
...
void ELECHOUSE_CC1101::Init(void)
{
  ...
  digitalWrite(SS_PIN, HIGH);   // SS is plain GPIO (-1 passed to SPI.begin), safe to drive
  // NOTE: Do NOT call digitalWrite(SCK_PIN) or digitalWrite(MOSI_PIN) here.
  // After SPI.begin(), those pins are owned by the SPI peripheral on ESP32-S3
  // with arduino-esp32 v3.x and can no longer be driven as plain GPIO.
  // The SPI peripheral manages SCK/MOSI idle state automatically.
  Reset();                    //CC1101 reset
  SpiEnd();                   // close SPI before RegConfigSettings: each SpiWriteReg opens
                              // its own SpiStart/SpiEnd, so nesting beginTransaction here
                              // would deadlock the non-reentrant SPI mutex.
