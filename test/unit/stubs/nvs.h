...
struct nvs_stats_t {
    size_t free_entries = 100;
    size_t total_entries = 100;
...
struct NvsStore {
    std::unordered_map<std::string, NvsEntry> entries;
};

// Namespace-keyed persistent store — survives open/close cycles within a test.
// Use nvs_stub_reset_ns(ns) or nvs_stub_reset_all() in test teardown.
...
inline void nvs_stub_reset_ns(const char *ns)
{
    nvs_ns_stores.erase(ns);
}
inline void nvs_stub_reset_all()
{
    nvs_ns_stores.clear();
    nvs_handle_ns.clear();
}
...
inline void nvs_close(nvs_handle_t h)
{
    nvs_handle_ns.erase(h);
}
...
{
    nvs_ns_stores[nvs_handle_ns[h]].entries.clear();
    return ESP_OK;
}
inline esp_err_t nvs_erase_key(nvs_handle_t h, const char *k)
{
    nvs_ns_stores[nvs_handle_ns[h]].entries.erase(k);
    return ESP_OK;
}
...
{
    auto &store = nvs_ns_stores[nvs_handle_ns[h]];
    auto it = store.entries.find(k);
    if (it == store.entries.end()) return ESP_FAIL;
...
#define NVS_DEFINE_SET(fn, ctype, nvs_type)                                                                            \
    ...
    {                                                                                                                  \
        auto &e = nvs_ns_stores[nvs_handle_ns[h]].entries[k];                                                          \
        e.type = nvs_type;                                                                                             \
        e.data.resize(sizeof(ctype));                                                                                  \
        memcpy(e.data.data(), &v, sizeof(ctype));                                                                      \
        return ESP_OK;                                                                                                 \
    }
...
#define NVS_DEFINE_GET(fn, ctype)                                                                                      \
    ...
    {                                                                                                                  \
        ...
        auto it = store.entries.find(k);                                                                               \
        if (it == store.entries.end()) return ESP_FAIL;                                                                \
        memcpy(v, it->second.data.data(), sizeof(ctype));                                                              \
        return ESP_OK;                                                                                                 \
    }
